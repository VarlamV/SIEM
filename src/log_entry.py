...
class LogEntry:
    ...
    def date_format(self):
        self.df['Date'] = pd.to_datetime(self.df['Date'].apply(lambda x: f"{x} {annee_de_depart}"),
                                         format='%b %d %H:%M:%S %Y',
                                         errors='coerce')
...
df['Annee'] = annee_de_depart
df['Mois'] = df['Date'].dt.month

for i in range(1, len(df)):
    if df.loc[i, 'Mois'] == 1 and df.loc[i-1, 'Mois'] == 12:
        df.loc[i:, 'Annee'] += 1

df['Date'] = df.apply(lambda row: row['Date'].replace(year=row['Annee']), axis=1)
df['Message'].fillna(value='No message', inplace=True)
df.loc[df['Message'].str.contains('error'), 'Info'] = 'error'
df.loc[df['Message'].str.contains('fail'), 'Info'] = 'fail'
df.loc[df['Message'].str.contains('succe'), 'Info'] = 'success'

df.to_csv("./data/Linux_log.csv")
